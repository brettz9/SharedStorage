(function () {
  ...
  */
    ...
    const _ = (key, formatObj) => {
      ...
      if (formatObj) {
        Object.entries(formatObj).forEach(([name, value]) => {
          message = message.replace(new RegExp(`\\{${pregQuote(name)}\\}`, 'gu'), value);
        });
      }
  ...
  const MEGABYTE = 1024 * 1000; // Async to support potential async `localStorage` in future: https://domenic.github.io/async-local-storage/

  const getMaximumRemainingStorage = async function () {
  ...
  */
  ...
  const SharedStorageTemplate = function ({
    _,
    prefs,
    boolPreferences,
    ...
    `], 'form', [['h2', [_('shared_storage_settings')]], ...boolPreferences.map(boolPref => {
  ...
  const namespaceKeyPreferences = ['noOrigin'];
  const originKeyPreferences = ['origins', 'namespacesWithOrigins'];
  const originKeySignallingExistencePreferences = ['originsGet', 'originsSet'];
  const objectPreferences = [...originKeyPreferences, ...namespaceKeyPreferences, ...originKeySignallingExistencePreferences];
  const prefs = {};
  const js = new JsonStorage({
    appNamespace: 'shared-storage'
  });
  ...
  async function setup() {
    await Promise.all([...objectPreferences, ...boolPreferences].map(async pref => {
  ...
  window.addEventListener('message', async function (e) {
    ...
    const postToOrigin = msgObj => {
      source.postMessage(Object.assign(msgObj, {
        id,
        attempt
      }), orign);
    };
    ...
    try {
      maxRemaining = await getMaximumRemainingStorage(); // Probably not a privacy concern to know the amount left, so we
      ...
      if (!{}.hasOwnProperty.call(data, 'data')) {
        ...
        if (!prefs.originsGet[orign]) {
          ...
          })).toLowerCase(); // 0. Remember? one for each site doing retrieving, one for
      ...
      postToOrigin({
        ...
        name,
        // 'NS_ERROR_DOM_QUOTA_REACHED' for storage limit
...
//# sourceMappingURL=SharedStorageServer.js.map
