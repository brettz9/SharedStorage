...
0. README todos:
  0. Add recommendation that a separate site be used to register the SharedStorage protocol (and set up in example)
  0. Namespaces with domain origins as keys and THEN subnamespaces (so easier to iterate custom namespaces with integrity)
  0. Use cases: 1) data ownership (e.g., social site, or any site which wants prefs, priv requests, content, to be full under (offline) user control); FF add-on to view localStorage for sites; review add-ons for localStorage and mention; idea for add-on to read/write localStorage via files (located anywhere) 2) Extensible application
  0. Options on whether the storage device will ask for user confirmation to avoid cross-domain spam/exploit attempts (even though apps ought to do their own checking too to the extent possible) and avoid size limits being broken
  0. Idea for Firefox add-on to add desktop listeners to file changes so, e.g., changes to a local file anywhere on the desktop could get reflected in local storage for a site (making data ownership even more meaningful)
...
window.addEventListener('message', function (e) {
  ...
  if (!origin) {
    throw new Error('No origin'); // Origin ought to be set by the browser; if there is a problem, the security of the origin-based data would be in jeopardy.
  }
  ...
  try {
    ...
    if (!mainData.hasOwnProperty('data')) { // Do this as opposed to checking truthiness since user might wish to set a falsey value
      ...
      if (!ls.originsGet[origin]) {
        prmpt = prompt('A site (' + (safeProtocol ? ' of supposed origin "' : 'of origin "') + origin + '") is attempting to retrieve shared data. Do you wish to approve? If you wish to always trust this site, type "t", if just for now, type "y". Otherwise, cancel. (From site "' + location.href + '"; namespace: "' + namespace + '"; namespacing type: "' + namespacing + '")').toLowerCase();
    ...
    if (!ls.originsSet[origin]) {
      prmpt = prompt('A site (' + (safeProtocol ? ' of supposed origin "' : 'of origin "') + origin + '") is attempting to set shared data. If you wish to always trust this site, type "t", if just for now, type "y". Otherwise, cancel. (Onto site "' + location.href + '"; namespace: "' + namespace + '"; namespacing type: "' + namespacing + '"; payload: "' + payload + '")').toLowerCase();
      if (prmpt === 't') {
