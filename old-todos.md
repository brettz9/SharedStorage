...
1. Make settings more than read-only; also could trigger storage events
1. Ensure working service worker added with update mechanism
  1. Detect storage size since API does not, based on string value length? if
    ...
    1. make queries to find space available/used (for non-confirm or confirm sites)
    1. Note: storage may need to take into account the key length too?
  1. Randomize which store site is used to avoid too many successive checks
  ...
  1. Devise scheme for packet distribution to better anonymize data
1. Custom protocol for shared storage itself
1. Add this code to an HTTPS site (to ensure at least the https origin keys
...
1. README todos:
  1. Add recommendation that a separate site be used to register the
    SharedStorage protocol (and set up in example)
  1. Namespaces with domain origins as keys and THEN subnamespaces
  ...
  1. Use cases:
    ...
    1. Extensible application (plugins, Web Intents, etc.)
  1. Options on whether the storage device will ask for user
  ...
  1. Idea for Firefox add-on to add desktop listeners to file
...
1. Subobjects: Allow retrieving and setting subobjects by array of
  property names?
1. Iteration of origins, namespaces, or namespaceOrigins (none of
...
1. Add optional AYW API to write storage to disk or retrieve data from disk?
1. Provide opportunity to clear()? Any real practical difference with
...
1. Need for IndexedDB (as for indexing)?
1. Add a demo: For domain-specific-enforcement forks (e.g., to minimize
...
1. Accept postMessage Transferable argument (ArrayBuffer and
  MessagePort) storage?
1. Allow querying of total memory size by saving all keys to temp
  variable, setting up to max, and then resetting back to tmp?
1. Placement on CDN like https://github.com/jsdelivr/api ?
...
1. Track a "lastSetBy" value to store the origin for non-origin
  items? Might encourae over-writing and deter more usage out of
  privacy concerns.
1. Auto-inform with error if string calculated to be long enough
  to overpass the limit? Will get an error if we try to go over anyways.
1. Require HTTPS scheme for origin-related storage (to prevent DNS
...
1. Expose even these preferences to requesting apps

1. Shared storage/add-on system
  1. use shared storage for file banks and have demo browse these
    hierarchically
  1. Same approach with trusted, offline HTTPS site confirming protocol and
...
1. Addon/eval
  1. Idea for evalInSandbox by submitting string for eval via postMessage
  ...
  1. find-and-replace memory/buttons (in a protocol manner to avoid being
    ...
    currently open one)
  1. addon to use Blockly for pipelining button output together
  1. Generic offline storage->registrar;  text editor as utilizer of storage
  ...
  1. The user visits a text editor add-ons site.
  1. The user clicks on a link with text like "Install toolbar button to
  ...
  1. The user is brought to the text editor web app (or if the link click
  ...
  1. The text editor web app (first checking the registrar if separate and
    ...
    is an eval install.
   1. If the user agrees, the text editor adds the button with label and
    ...
    1. Opens the site in a hidden iframe, and uses the same API for
